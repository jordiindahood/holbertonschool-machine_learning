...
def densenet121(growth_rate=32, compression=1.0):
    ...
    my_layer = K.layers.MaxPooling2D(
    ...
    for layer in layers:
        my_layer, nb_filters = transition_layer(
            my_layer, nb_filters, compression
        )

        my_layer, nb_filters = dense_block(
            my_layer, nb_filters, growth_rate, layer
        )

    my_layer = K.layers.AveragePooling2D(pool_size=(7, 7))(my_layer)

    my_layer = K.layers.Dense(
