...
class WGAN_GP(keras.Model):
    ...
    def train_step(self, useless_argument):
        ...
        for _ in range(self.disc_iter):
            with tf.GradientTape() as tape:
                ...
                interpolated = self.get_interpolated_sample(real_samples, fake_samples)
    ...
    def replace_weights(self, gen_h5, disc_h5):
        ...
        with the ones stored in .h5 files.

        Args:
            gen_h5 (str): Path to generator weights (.h5)
            disc_h5 (str): Path to discriminator weights (.h5)
        """
        try:
            self.generator.load_weights(gen_h5)
            print(f"Generator weights loaded from {gen_h5}")
        except Exception as e:
            print(f"Error loading generator weights: {e}")

        try:
            self.discriminator.load_weights(disc_h5)
            print(f"Discriminator weights loaded from {disc_h5}")
        except Exception as e:
